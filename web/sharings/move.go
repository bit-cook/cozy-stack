...
func moveFileFromSharedDriveCore(inst *instance.Instance, sourceInstanceURL string, fileID string,
	...
	srcClient := client.NewFilesClient(NewRemoteClient(u, bearer), s.ID())
...
func remoteContentToMove(remoteClient *client.FilesClient, dirID string) ([]*client.DirOrFile, []*client.DirOrFile, error) {
...
func moveDirBetweenSharedDrives(c echo.Context, sourceInstanceURL, sourceDirID string, sourceSharing *sharing.Sharing, destInstanceURL, dirID string, destSharing *sharing.Sharing, copy bool) error {
	...
	sourceClient := client.NewFilesClient(NewRemoteClient(sourceURL, sourceBearer), sourceSharing.ID())
	...
	destClient := client.NewFilesClient(NewRemoteClient(destURL, destBearer), destSharing.ID())
...
var NewSharedDriveClient = func(s *sharing.Sharing) (*client.FilesClient, error) {
	...
	return client.NewFilesClient(NewRemoteClient(destURL, bearer), s.ID()), nil
}
...
func ensureRemoteChildDir(c *client.FilesClient, parent *client.Dir, name string) (string, error) {
...
func ensureRemoteUniqueChildName(c *client.FilesClient, parentFullpath, name string, isFile bool) (string, error) {
	candidate := name
...
func uploadWithConflictRetry(c *client.FilesClient, parentFullpath, dirID, name string, md5sum []byte, contents io.Reader, contentType string, contentLength int64) (*client.File, error) {
